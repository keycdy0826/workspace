Page({
  ...
  async onShow(){
    ...
    this.getUser()
    await wx.cloud.callFunction({name: 'getList', data: {list: getApp().globalData.collectionMarketList}}).then(async data => {
      const allItems = data.result.data
      const withImages = allItems.filter(i => i.image && !i.imageUrl)
      if (withImages.length > 0) {
        const fileList = withImages.map(i => i.image)
        await wx.cloud.getTempFileURL({ fileList }).then(res => {
          const map = {}
          for (const f of res.fileList) map[f.fileID] = f.tempFileURL
          for (const i of withImages) i.imageUrl = map[i.image]
        })
      }
